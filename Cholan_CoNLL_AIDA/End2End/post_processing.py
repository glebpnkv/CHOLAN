...
def combine_sentence(df_target, df_predicted, predict_data_dir):
    ...
    df_predicted_sent = df_predicted.groupby('Sentence', as_index=False).agg(
        {'EntityMention': '|'.join, 'Predicted_Wikilabel': '|'.join}
    )
    df_target_sent = df_target.groupby('Sentence', as_index=False).agg(
        {'EntityMention': '|'.join, 'Target_Wikilabel': '|'.join}
    )

    # Get target sentences for the corresponding predictions
    df_final_target = df_target_sent[
        ((df_target_sent['Sentence'].str.strip()).isin(df_predicted_sent['Sentence'].str.strip()))
    ]
